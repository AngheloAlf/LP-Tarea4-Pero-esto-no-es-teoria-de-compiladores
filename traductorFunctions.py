...
def nombreValorValido(linea):
    if lineaVacia(linea): sintax_error("No hay valor para asignar a la variable")
    ...
    else: sintax_error("Asignacion de variable invalida")
    return
...
def generarAsignacion(linea):
    ...
    if len(asignacionIzq)>1:
        ...
        if len(variable) == 1: sintax_error("Asignacion de una variable sin el uso de VAR")
        variable = variable[1]
...
def revisarProcInicio(linea, PROC):
    ...
    if PROC and re.search("\$\^PROC", linea): sintax_error("Funcion dentro de funcion")
    if PROC: return PROC, None
    if re.search("\$\^PROC\s*\(", linea): return True, nombre[1]
    return False, None

def revisarProcFin(linea, PROC):
    if not PROC and re.search("\^\$", linea): sintax_error("Cierre de funcion sin entrar en ella")
